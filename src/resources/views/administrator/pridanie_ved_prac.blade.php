                    ...
                    @unless($users->isEmpty())
                        @foreach ($users as $ved)
                            <tr>
                                ...
                                {{-- pridanie zamestnanca do zoznamu vedúcich pracovísk --}}
                                <td class="py-2 text-l border-b">
                                    <form method="POST" action="/pridanie_ved_prac/{{ $ved->id }}">
                                        @csrf
                                        @method('PUT')
                                        <button class="hover:text-green-500"><i
                                                class="fa-solid fa-user-plus"></i> Pridať</button>
                                    </form>
                                </td>
                            </tr>
                        @endforeach
